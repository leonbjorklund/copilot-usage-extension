...
- **Hover limits:** the hover keeps tables, `img` tags with `data:` URIs and `width`/`height`/`title`, and `span` color styles. It stays open under the mouse only while it contains a link. Each `statusBar.text` or `tooltip` assignment sends the window an update; VS Code redraws an open hover only when the markdown differs.
