...
- After code changes, run `npm run preview` to rebuild and open the development preview with the user's normal VS Code profile, real usage logs, and live quota.
...
- Background quota requests use `getSession` with an empty scope list and `silent: true`, with `account` set to the login `copilotAccount.ts` reports so the row follows Copilot. `createIfNone` is allowed for `copilotUsage.connectQuota` and once per account after a new successful panel chat request in this window. The automatic offer requires a known matching signed-in account and a persisted exclusive claim under `context.globalStorageUri/quota-consent`, so cancellation, reloads, and concurrent windows cannot repeat it. Only the manual command may pair an unknown login with `clearSessionPreference` to show the account picker. Never pass `forceNewSession`: it forces a fresh sign-in even when the existing session is fine, and any session this extension creates itself carries no scopes. Reuse the sessions Copilot already holds, which the picker does.
