...
type Bill = { kind: 'bill'; key: string; record: UsageRecord; sessionStart?: number };
...
/** Quota capacity stays live; the spent percentage uses only tracked usage. */
export function trackedUsageQuota(quota: CopilotQuota, summary: UsageSummary): CopilotQuota {
  const spent = summary.month.githubCopilot.aiCredits;
  const remaining = Math.max(0, quota.entitlement - spent);
  return { ...quota, remaining,
    percentRemaining: quota.unlimited ? 100 : quota.entitlement > 0 ? remaining / quota.entitlement * 100 : 0,
    overageCount: Math.max(0, spent - quota.entitlement) };
}
...
export class AccountUsagePoc {
  ...
  refresh(summary: UsageSummary, now = new Date()): Promise<AccountPocView> {
    const work = this.queue.then(() => this.refreshOnce(summary, now));
    this.queue = work.catch(() => undefined);
    return work;
  }

  private async refreshOnce(summary: UsageSummary, now: Date): Promise<AccountPocView> {
    ...
    for (const file of logFiles) {
      try {
        ...
        for (const entry of parseAccountEvidence(text, resolve(dirname(file)))) {
          if (isAuth(entry) || entry.at >= this.startedAt) this.add(entry, additions);
        }
      } catch {
    ...
    for (const entry of this.entries.values()) {
      ...
      replaced.set(entryKey(entry), entry);
      this.add({ ...entry, sessionStart }, additions);
    }
    for (const chat of summary.chats) {
      for (const record of chat.records) {
        ...
        if (this.entries.has(billKey(record, true))) continue;
        const sessionStart = await this.readSessionStart(record.filePath);
        this.add({ kind: 'bill', key, record: { ...record, title: chat.title }, sessionStart }, additions);
      }
    }
    if (additions.length) {
    ...
    for (const entry of bills) {
      ...
      if ('account' in decision) {
        if (decision.account === account) records.push(entry.record);
      } else {
  ...
  private add(entry: Entry, additions?: Entry[]): void {
    const key = entryKey(entry);
    const previous = this.entries.get(key);
    if (previous) {
      if (previous.kind !== 'bill' || entry.kind !== 'bill' || previous.sessionStart !== undefined || entry.sessionStart === undefined) return;
      entry = { ...previous, sessionStart: entry.sessionStart };
    }
