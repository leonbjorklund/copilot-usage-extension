...
Log reading is entirely local and no usage data leaves your machine. The only endpoint the extension contacts is your own AI Credit quota at GitHub, read with the GitHub account Copilot Chat is signed in with: the extension takes that account from the "Logged in as" line in Copilot Chat's own log for this window, so switching Copilot's account switches the row within a few seconds. Click the top row of the tree to grant access once per account; until you do, no request is made. After that the row re-reads on startup and shortly after Copilot bills new credits, at most once a minute. If the log names no account, clicking the row lets you pick one by hand. GitHub Enterprise Server accounts are not supported. That endpoint is the unofficial one Copilot Chat itself uses, so the row can stop reporting if GitHub changes it; nothing else in the extension depends on it.

The quota row shows credits left, the credits included in your plan, the percentage remaining, and the account. Its tooltip adds credits used, the reset date, and any overage. An account with no Copilot subscription gets no row.
